...
void Arduino_ESP32RGBPanel::rotate(ESP32RGBPanel_rotation_t rotation) {
  switch (rotation)
  {
  case ESP32RGBPanel_ROTATION_NORMAL:
    break;
  case ESP32RGBPanel_ROTATION_LANDSCAPE_MIRROR:
    if(esp_lcd_panel_mirror(_panel_handle, false, true) == ESP_ERR_NOT_SUPPORTED)
      printf("Rotate not supported!\n");
    break;
  case ESP32RGBPanel_ROTATION_PORTTRAIL:
    if(esp_lcd_panel_swap_xy(_panel_handle, true) == ESP_ERR_NOT_SUPPORTED)
      printf("Rotate not supported!\n");
    break;
  case ESP32RGBPanel_ROTATION_PORTTRAIL_MIRROR:
    break;
  }
}
